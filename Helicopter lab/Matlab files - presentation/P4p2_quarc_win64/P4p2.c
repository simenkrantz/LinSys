 ...
 * C source code generated on : Wed Oct 17 18:22:27 2018
 *
...
void P4p2_output0(void)                /* Sample time: [0.0s, 0.0s] */
{
  ...
  if (rtmIsMajorTimeStep(P4p2_M)) {
     ...
     */
    P4p2_B.Sum = P4p2_B.Gain_e - P4p2_P.Elevationconstant_Value;
  }
  ...
  for (i = 0; i < 6; i++) {
    tmp_0[i] = P4p2_P.Gain1_Gain * tmp[i];
  }
  ...
  for (i = 0; i < 3; i++) {
    rtb_Sum1[i] = 0.0;
    for (i_0 = 0; i_0 < 6; i_0++) {
      rtb_Sum1[i] += P4p2_P.C[3 * i_0 + i] * tmp_0[i_0];
    }
  }

  if (rtmIsMajorTimeStep(P4p2_M)) {
    /* Constant: '<Root>/Constant1' */
    P4p2_B.Constant1 = P4p2_P.Constant1_Value;
   ...
   */
  for (i = 0; i < 2; i++) {
    rtb_Sum5[i] = P4p2_B.P[i] - ((P4p2_P.K[i + 2] * P4p2_X.Integrator2_CSTATE[1]
      + P4p2_P.K[i] * P4p2_X.Integrator2_CSTATE[0]) + P4p2_P.K[i + 4] *
      P4p2_X.Integrator2_CSTATE[3]);
  }

  /* End of Sum: '<Root>/Sum5' */

  /* Sum: '<Root>/Sum6' */
  rtb_Backgain = rtb_Sum5[0] + P4p2_B.Constant1;
  if (rtmIsMajorTimeStep(P4p2_M)) {
  }
   ...
   *  Sum: '<S1>/Add'
   */
  u0 = (rtb_Sum5[1] + rtb_Backgain) * P4p2_P.Frontgain_Gain;
   ...
   *  Sum: '<S1>/Subtract'
   */
  u0 = (rtb_Backgain - rtb_Sum5[1]) * P4p2_P.Backgain_Gain;
  ...
  for (i = 0; i < 3; i++) {
    rtb_Backgain = 0.0;
    for (i_0 = 0; i_0 < 6; i_0++) {
      rtb_Backgain += P4p2_P.C[3 * i_0 + i] * P4p2_X.Integrator2_CSTATE[i_0];
    }
  ...
  for (i = 0; i < 6; i++) {
    ...
    tmp[i] = 0.0;
    for (i_0 = 0; i_0 < 6; i_0++) {
      tmp[i] += P4p2_P.A[6 * i_0 + i] * P4p2_X.Integrator2_CSTATE[i_0];
    }
     ...
     */
    tmp_0[i] = 0.0;
    tmp_0[i] += P4p2_P.B[i] * rtb_Sum5[0];
    tmp_0[i] += P4p2_P.B[i + 6] * rtb_Sum5[1];

    /* Sum: '<S5>/Sum9' incorporates:
     *  Gain: '<S5>/L'
     */
    P4p2_B.Sum9[i] = ((P4p2_P.L[i + 6] * rtb_Sum1[1] + P4p2_P.L[i] * rtb_Sum1[0])
                      + P4p2_P.L[i + 12] * rtb_Sum1[2]) + (tmp[i] + tmp_0[i]);
  }
...
void P4p2_derivatives(void)
{
  ...
  for (i = 0; i < 6; i++) {
    _rtXdot->Integrator2_CSTATE[i] = P4p2_B.Sum9[i];
  }
...
RT_MODEL_P4p2_T *P4p2(void)
{
  ...
  /* External mode info */
  P4p2_M->Sizes.checksums[0] = (468595011U);
  P4p2_M->Sizes.checksums[1] = (97474990U);
  P4p2_M->Sizes.checksums[2] = (1367399080U);
  P4p2_M->Sizes.checksums[3] = (1179443262U);
  ...
  P4p2_M->Sizes.numBlocks = (56);      /* Number of blocks */
  P4p2_M->Sizes.numBlockIO = (21);     /* Number of block outputs */
  P4p2_M->Sizes.numBlockPrms = (239);  /* Sum of parameter "widths" */
  return P4p2_M;
}
